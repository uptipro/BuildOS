...
function scheduleInfo(plannedEndDate: string): {
  label: string;
  color: string;
} {
  const now = Date.now();
...
export function PortfolioDashboardPage() {
  ...
  const stats = useMemo(() => {
    const onTrack = activeProjects.filter(
      (p) => p.ragStatus === "on-track",
    ).length;
    const atRisk = activeProjects.filter(
      (p) => p.ragStatus === "at-risk",
    ).length;
    const delayed = activeProjects.filter(
      (p) => p.ragStatus === "delayed",
    ).length;
    ...
    return [
      ...
      },
      {
        ...
        value: onTrack,
        dot: true,
        accent: RAG_HEX["on-track"],
      },
      {
        label: "At Risk",
        value: atRisk,
        dot: true,
        accent: RAG_HEX["at-risk"],
      },
      {
        label: "Delayed",
        value: delayed,
        dot: true,
        accent: RAG_HEX["delayed"],
      },
      {
        label: "Total Budget",
  ...
  const filtered = useMemo(() => {
    ...
    return base.filter((p) => {
      if (clusterFilter !== "All" && p.clusterId !== clusterFilter)
        return false;
      ...
      if (dateFrom && new Date(p.plannedStartDate) < new Date(dateFrom))
        return false;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: "#94A3B8" }}>
              Executive overview &middot; {activeProjects.length} active
              projects
            </p>
          </div>
        </div>
        ...
        >
          ...
          {(["on-track", "at-risk", "delayed"] as const).map((r) => {
            ...
            const pct = Math.round(
              (count / Math.max(activeProjects.length, 1)) * 100,
            );
            return (
              <div key={r} className="flex items-center gap-3 mb-2">
                <span
                  className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                  style={{ backgroundColor: RAG_HEX[r] }}
                />
                <span className="text-sm flex-1" style={{ color: "#718096" }}>
                  {r === "on-track"
                    ? "On Track"
                    : r === "at-risk"
                      ? "At Risk"
                ...
                <div className="w-24 bg-gray-100 rounded-full h-2 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${pct}%`, backgroundColor: RAG_HEX[r] }}
                  />
                </div>
            ...
            return (
              <>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span
                    className="text-sm font-bold"
                    style={{ color: remaining >= 0 ? "#27AE60" : "#E74C3C" }}
                  >
                ...
                <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        pct > 90 ? "#E74C3C" : pct > 75 ? "#F4A623" : "#27AE60",
                    }}
                  />
                </div>
                <div
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span
                    ...
                    style={{
                      backgroundColor: RAG_BG_HEX[project.ragStatus],
                      color: RAG_HEX[project.ragStatus],
                    }}
                  >
                    <span
                      className="w-1.5 h-1.5 rounded-full"
                      style={{ backgroundColor: RAG_HEX[project.ragStatus] }}
                    />
                ...
                <div>
                  ...
                  >
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        backgroundColor: RAG_HEX[project.ragStatus],
                      }}
                    />
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {project.lastReportDate ? (
                      <>
                        {Math.floor(
                          (Date.now() -
                            new Date(project.lastReportDate).getTime()) /
                            86400000,
                        ) > 1 && (
                          <AlertTriangle
                            className="w-3 h-3 flex-shrink-0"
